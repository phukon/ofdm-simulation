...
im_bin = dec2bin(im(:))';
% Each element in the column in represented in binary, hence the matrix is of order mXn. After which it is transposed, so each column represents the
% original data in binary. And the data is read like a book, from left to
% right.
%The column stores the value of the N-bit blocks of data
im_bin = im_bin(:); %Packed into a column. It's a waterfall stream now! Now a train of bits represent the data
